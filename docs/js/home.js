  ...
  const projects = {
    ...
    chart: {
      ...
      background: [
        ...
        '业务侧：只做图表生成容易停留在好看，无法保证字段来源和结论可靠',
        '系统侧：需要把图表推荐、数据结构、字段来源和规则校验组织成可验证链路'
      ],
      goalMetrics: [
        '用户指标：输入业务判断后，系统能推荐合适图表模型并生成可编辑可视化',
        '质量指标：图表类型、字段映射、数值范围和结构完整性必须通过检查',
        '证据指标：关键字段能回溯来源，数据口径不足时明确标注缺口'
      ],
      action: [
        '产品链路：把用户输入拆成表达意图、图表模型、字段结构和校验结果',
        '模型库设计：沉淀定位图、雷达图、热力图、漏斗图、桑基图、流程图等图表模型和推荐规则',
        '执行拆分：设计 Agent 进度状态、结构化图表 JSON、硬规则校验和字段来源面板',
        '验收方式：用字段来源、结构检查和图表可编辑性作为可交付标准，而不只看生成是否好看'
      ],
      evidence: [
        '本地项目位于 /Users/kanyun/projects/chart-ai，已形成可运行 Next.js 原型',
        '可与桌面研究项目串联使用，但本项目独立承担图表模型推荐、字段校验和可视化交付',
        '案例证明的是“图表模型推荐 + 字段校验 + 可编辑表达”的 Agent 产品能力，不是简单图表生成'
      ],
    },
    venn: {
      category: '研究交付工作流',
      ...
      evidence: [
        ...
        '它和 CHART AI、桌面研究放在一起，能体现不同研究表达场景的产品判断'
      ],
    ...
    birdcircle: {
      ...
      caseStudy: {
        ...
        insights: [
          {
            label: '观鸟',
            text: '人群洞察：观鸟和拍鸟首先是两个群体，有各自的行为方式和群体认同。\n\n🔭观鸟一个望远镜就可以开始，更多地关注鸟的珍稀种类和自己本身的行为，比如我的好朋友讲到，雄性翠鸟的嘴巴是纯黑的，雌性的嘴巴是上黑下黄，所以会说翠鸟会“涂口红”。\n\n🎈这类群体的需求上偏向于收集，学科上偏向于博物学。'
          },
          {
            label: '拍鸟',
