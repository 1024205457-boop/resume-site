...
(function () {
  ...
  const projects = {
    ...
    zebra: {
      ...
      background: '体验课后的家长跟进高度依赖人工顾问经验，常见问题包括提醒断点、学情反馈口径不一致、课程推荐时机不稳定和购买答疑重复。',
      action: '梳理体验会员链路，将到课提醒、学习反馈、阶段判断、课程推荐、异议处理和购买答疑拆成可复用服务节点，并设计 AI 顾问的意图识别与回复策略。',
      goal: '让 AI 顾问承接体验会员从课后跟进到转化线索沉淀的连续服务，提高回复稳定性和运营可控性。',
      result: '形成脱敏的智能学习顾问方案，可用用户旅程图、对话样例、意图分类表和服务流程图展示完整产品思路。'
    },
    chart: {
      category: 'AI Agent 产品',
      ...
      background: '商业分析材料经常停留在文字、表格或零散研究结论中，转成定位图、漏斗图、桑基图等可汇报表达时，需要反复判断图表类型、补数据和校验口径。',
      action: '搭建 Chart-AI 原型，设计图表模型库、推荐规则、Agent 进度状态、桌面研究补数、结构化图表生成、硬规则校验和字段来源追溯。',
      goal: '把“我要表达一个商业判断”变成可选图表模型、可补充研究事实、可编辑且可追溯的数据可视化流程。',
      result: '已形成 Next.js 原型，支持定位图、雷达图、热力图、漏斗图、桑基图、流程图、维恩图等模型，并能展示字段来源和生成检查结果。'
    },
    content: {
      category: 'AI 工作流自动化',
    ...
    busycall: {
      ...
      background: '远程会议和线上沟通中，用户有时需要自然离场，但直接打断或找理由容易尴尬。',
      action: '通过 4 场用户访谈确认痛点，参考真实来电弹窗设计高仿真来电界面，并用快速开发工具完成触发流程。',
      goal: '提供一个轻量、即时、可信的社交缓冲工具，让用户可以自然结束不合适的线上场景。',
      result: '项目 3 小时内完成开发上线，积累 300+ 用户，并在社交平台获得 6w+ 关注。'
    },
    invoice: {
      category: '更多项目 · 业务工具',
      title: '发票金额填报与汇总工具',
  ...
  function openProject(card) {
    ...
    fields.action.textContent = project.action || project.role;
