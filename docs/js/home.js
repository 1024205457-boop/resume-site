...
(function () {
  ...
  const projects = {
    ...
    chart: {
      ...
      summary: '承接 MRW 产出的可信洞察，把“我要表达一个商业判断”转成可选择图表模型、可校验字段来源的结构化可视化交付。',
      role: '设计 Chart-AI 产品链路、图表模型库、图表推荐规则、Agent 进度状态、硬规则校验和字段来源追溯。',
      intro: 'Chart AI 是研究交付工作流的后环。前环 MRW 负责把 Brief、原始材料和用户反馈整理成有证据链的洞察报告；后环 Chart AI 承接这些洞察，理解分析目标并推荐图表模型，再根据定位图、雷达图、热力地图、漏斗图、桑基图、流程图、维恩图等模型生成结构化图表数据。生成后继续检查图表类型、字段映射、数值范围和结构完整性，让研究结论进入可编辑、可追溯的汇报表达。',
      demo: '当前可放 Chart-AI 运行录屏、图表模型选择页、字段来源面板和生成结果截图；MRW 的样例报告可作为“研究结论进入可视化表达”的延展 Demo。',
      background: [
        '用户侧：咨询分析师和产品经理已经有洞察结论，但不知道该用什么图表达、缺哪些字段、口径是否自洽',
        '业务侧：只做研究报告难以进入可编辑汇报表达，只做图表生成又无法保证字段来源和结论可靠',
        '系统侧：需要把 MRW 的证据链和 Chart AI 的图表生成合成一条前后环可验证链路'
      ],
      goalMetrics: [
      ...
      action: [
        '产品链路：把 MRW 定位为“前环洞察生产”，把 Chart AI 定位为“后环表达交付”',
    ...
    mrw: {
      ...
      summary: '把分散研究材料整理成有来源、有聚类、有结论的桌面研究报告，作为 Chart AI 可视化交付的上游输入。',
      role: '设计从 Brief、样本整理、主题聚类、证据引用到报告输出的研究工作流。',
      intro: 'MRW 是研究交付工作流的前环：先解决“研究材料怎么变成可信洞察”的问题，再把结果交给 Chart AI 做可视化表达。流程从研究 Brief 出发，整理样本材料和事实来源，做主题聚类、用户画像、痛点 Top、需求机会和策略建议，并通过源校验、交叉校验和逻辑校验保证关键洞察能回溯证据。',
      ...
      action: [
        ...
        '前后环连接：把这个工作流作为 Chart AI 的上游，让研究结论继续进入图表表达'
      ],
      evidence: [
        '已生成敏感肌护肤品类消费者洞察样例报告',
    ...
    frame: {
      ...
      background: [
        '用户侧：年轻用户需要低门槛表达和记录情绪，但纯聊天陪伴难以形成稳定练习闭环',
        '业务侧：情绪产品需要把表达、分析和建议组织成一次可完成、可回看的练习流程',
        '系统侧：视频上传、音频转写、语音情绪、视频情绪识别和 LLM 生成需要被拆成稳定链路'
      ],
      goalMetrics: [
        '用户指标：用户能通过三层引导完成一次情绪表达和记录',
        '产品指标：形成“视频记录 + 多模态分析 + 个性化建议”的核心原型',
        '验证指标：路演现场验证试用意愿和孵化兴趣'
      ],
      action: [
        '系统架构：搭建 React Native + Expo 前端和 FastAPI 后端，拆分认证、视频处理、引导问题、建议生成和记忆管理接口',
        '多模态链路：将用户视频拆成音频、画面和文本三路信号，串联 MoviePy 音频提取、DashScope ASR、YOLO11 视频情绪识别和转录文本分析',
        'LLM 编排：用 LangChain 管理三层递进式 Prompt，把开放聊天约束为事件探索、感受细化、情绪认知的稳定引导流程',
        '长期跟踪：把三轮回答、情绪标签和时间戳摘要为用户记忆，用于历史情绪记录、用户画像沉淀和后续个性化问题生成'
      ],
      evidence: [
        '项目路演时获得 2 家孵化器支持',
        '200+ 观众中 69 人愿意试用',
        'GitHub 中保留 FastAPI 后端、Expo 前端、LangChain 引导与建议生成、多模态情绪分析相关代码'
      ],
